...
function updateAnim() {
  ...
  if (disp)
    disp.setAttribute('scale', String(30 * (props.chaos || 1)));
...
<template>
  <div ref="rootRef" class="relative isolate flex flex-col h-full w-full" :class="[className]" :style="style">
    <svg
      ref="svgRef"
    ...
    <div class="relative flex flex-col flex-1 w-full" :style="inheritRadius">
      <slot />
    </div>
  </div>
</template>
